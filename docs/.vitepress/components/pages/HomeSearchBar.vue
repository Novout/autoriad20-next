<template>
  <div class="flex rounded-lg items-center mr-0 md:mr-20 shadow-lg" :style="{ backgroundColor: 'rgba(0, 0, 0, 0.2)'}">
    <input v-model="input" class="pl-2 py-1" type="text" />
    <div class="pr-2">
    ...
    <div :style="{ backgroundColor: 'var(--vp-c-bg-alt)'}" class="absolute top-12 w-60 rounded max-h-50 overflow-y-auto">
      <div class="truncate cursor-pointer font-bold p-2" @click.prevent.stop="onNav(target)" v-for="(target, index) in targets" :key="index">
        {{ target.name }}
      </div>
    </div>
  </div>
...
const onNav = ({ name, url }: Record<string, any>) => {
  window.location.href = `${url}?component=${name}`
}
</script>